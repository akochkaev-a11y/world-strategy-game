...
func _process(delta: float) -> void:
    ...
    # Для прототипа: 1 реальная секунда = 1 игровая минута.
...
func _build_ui() -> void:
    ...
    var right_scroll := ScrollContainer.new(); right_scroll.custom_minimum_size.x = 330
    selected_panel = VBoxContainer.new(); selected_panel.custom_minimum_size.x = 310
    right_scroll.add_child(selected_panel); body.add_child(right_scroll)

    var bottom := HBoxContainer.new(); bottom.custom_minimum_size.y = 54; root.add_child(bottom)
    var save_btn := Button.new(); save_btn.text = "Сохранить"; save_btn.pressed.connect(_save_game); bottom.add_child(save_btn)
    var load_btn := Button.new(); load_btn.text = "Загрузить"; load_btn.pressed.connect(_load_game); bottom.add_child(load_btn)
    var ng_btn := Button.new(); ng_btn.text = "Новая партия"; ng_btn.pressed.connect(_new_game); bottom.add_child(ng_btn)
    var note := Label.new(); note.text = "  1 секунда = 1 игровая минута (режим тестирования)"; bottom.add_child(note)
...
func _refresh_country_list() -> void:
    ...
    var head := Label.new(); head.text = "СТРАНЫ"; head.add_theme_font_size_override("font_size", 20); countries_box.add_child(head)
    for id in countries.keys():
        var c: Dictionary = countries[id]
        var b := Button.new()
        b.text = "%s   [%d]" % [c.name, int(_total_power(c))]
        if id == player_id: b.text += "  ВЫ"
...
func _refresh_army() -> void:
    ...
    for key in UNIT_KEYS:
        var row := HBoxContainer.new(); army_box.add_child(row)
        var l := Label.new(); l.text = "%s: %d" % [UNIT_LABELS[key], int(countries[player_id][key])]; l.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(l)
        var b := Button.new(); b.text = "+100 за %.0f млн" % _unit_price(player_id, key); b.pressed.connect(_buy_unit.bind(key)); row.add_child(b)
...
func _add_label(parent: Node, text: String, size := 16) -> void:
    var l := Label.new(); l.text = text; l.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; l.add_theme_font_size_override("font_size", size); parent.add_child(l)
...
func _economic_tick() -> void:
    for id in countries.keys():
        var c: Dictionary = countries[id]
        var upkeep := _total_power(c) * 0.00008
...
func _bot_may_attack(attacker_id: String) -> void:
    var candidates := countries.keys().filter(func(x): return x != attacker_id)
    ...
    var threshold := 1.5
    if a.ai == "агрессивный": threshold = 1.15
    elif a.ai == "осторожный": threshold = 1.8
...
func _unit_price(id: String, key: String) -> float:
    var c: Dictionary = countries[id]
    var growth := 1.0 + float(c[key]) / 20000.0
    return UNIT_COST[key] * growth

func _buy_unit(key: String) -> void:
    var p: Dictionary = countries[player_id]
    var price := _unit_price(player_id, key)
    if p.treasury < price:
...
func _open_attack_dialog(target: String) -> void:
    ...
    for key in UNIT_KEYS:
        ...
        var l := Label.new(); l.text = UNIT_LABELS[key]; l.custom_minimum_size.x = 130; row.add_child(l)
        var slider := HSlider.new(); slider.min_value = 0; slider.max_value = 100; slider.step = 5; slider.value = 25; slider.custom_minimum_size.x = 260; row.add_child(slider)
        var val := Label.new(); val.text = "25%"; val.custom_minimum_size.x = 55; row.add_child(val)
    ...
    dialog.confirmed.connect(func():
        ...
        var avg_fraction := clamp(weighted / UNIT_KEYS.size(), 0.05, 1.0)
...
func _resolve_battle(attacker_id: String, defender_id: String, attack_fraction: float, silent_bot := false) -> void:
    var a: Dictionary = countries[attacker_id]
    var d: Dictionary = countries[defender_id]
    var defense_fraction := _ai_defense_fraction(d)
    var attack_power := _combat_power(a, attack_fraction, false) * randf_range(0.90, 1.10)
    var defense_power := _combat_power(d, defense_fraction, true) * randf_range(0.90, 1.10)
    var ratio := attack_power / max(1.0, defense_power)
    var a_loss := 0.12
    var d_loss := 0.12
    var winner := ""
    if ratio > 1.5:
    ...
    var destroyed_value := 0.0
    for key in UNIT_KEYS:
        var av := float(a[key]) * attack_fraction
        var dv := float(d[key]) * defense_fraction
        var al := av * a_loss
        var dl := dv * d_loss
    ...
    if winner != "":
        ...
        _push_news("%s побеждает в столкновении с %s. Награда %.0f млн." % [countries[winner].name, countries[defender_id if winner == attacker_id else attacker_id].name, destroyed_value * 0.5])
    else:
...
func _ai_defense_fraction(c: Dictionary) -> float:
    match c.ai:
        "осторожный": return randf_range(0.55, 0.85)
        "агрессивный": return randf_range(0.30, 0.60)
        "авантюрный": return randf_range(0.20, 0.80)
        _: return randf_range(0.40, 0.70)

func _combat_power(c: Dictionary, fraction: float, defender: bool) -> float:
    var ground := float(c.army) * fraction
    var air := float(c.air) * fraction
    var navy := float(c.navy) * fraction
    var defense := float(c.def) * fraction
    var missile := float(c.missile) * fraction
    var air_support := air * (1.0 - min(0.55, defense / max(1.0, air + 5000.0)))
    var total := ground + air_support * 0.9 + navy * 0.45 + missile * 0.65 + defense * (0.7 if defender else 0.25)
    if defender: total *= 1.15
    total *= 1.0 - float(c.war_fatigue) / 250.0
    return total

func _total_power(c: Dictionary) -> float:
    return float(c.army + c.air + c.navy + c.def + c.missile)

func _push_news(text: String) -> void:
    news.push_front(text)
    if news.size() > 25: news.resize(25)
    if is_instance_valid(news_box): _refresh_news()

func _save_game() -> void:
    var data := {"countries": countries, "player_id": player_id, "selected_id": selected_id, "news": news}
...
func _load_game() -> void:
    ...
    if typeof(data) == TYPE_DICTIONARY:
        ...
        news.clear()
        for x in data.get("news", []): news.append(str(x))
