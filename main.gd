...
func _resolve_battle(attacker_id: String, defender_id: String, attack_fraction: float, silent_bot: bool = false) -> void:
    ...
    if winner != "":
        ...
        _push_news(
            ...
            [countries[winner].name, countries[defender_id if winner == attacker_id else attacker_id].name, destroyed_value * 0.5]
        )
    else:
...
func _combat_power(c: Dictionary, fraction: float, defender: bool) -> float:
    ...
    var total: float = ground + air_support * 0.9 + navy * 0.45 + missile * 0.65 + defense * (0.7 if defender else 0.25)
...
func _save_game() -> void:
    var data: Dictionary = {"countries": countries, "player_id": player_id, "selected_id": selected_id, "news": news}
