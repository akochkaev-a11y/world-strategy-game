...
func _build_ui() -> void:
    ...
    var troop_btn := Button.new(); troop_btn.text = "ВОЙСКА"; troop_btn.custom_minimum_size = Vector2(105, 44); troop_btn.pressed.connect(func(): if selected_id != player_id: _open_attack_dialog(selected_id)); map_action_bar.add_child(troop_btn)
    ...
    var help_btn := Button.new(); help_btn.text = "ПОМОЩЬ"; help_btn.custom_minimum_size = Vector2(105, 44); help_btn.pressed.connect(func(): if selected_id != player_id: _send_aid(selected_id)); map_action_bar.add_child(help_btn)
...
func _refresh_army() -> void:
    ...
    for key in UNIT_KEYS:
        ...
        var l := Label.new(); l.text = "%s: %d (без личного состава)" % [UNIT_LABELS[key], int(countries[player_id][key])] if key == "missile" else "%s: %d чел." % [UNIT_LABELS[key], int(countries[player_id][key])]; row.add_child(l)
...
func _buy_unit_amount(key: String, amount: int) -> void:
    ...
    if float(p.treasury) < price: _push_news("Недостаточно средств для покупки: %s x%d." % [UNIT_LABELS[key], amount]); return
    if not _can_recruit(p, key, float(amount)): _push_news("Недостаточно свободного населения для увеличения «%s» на %d человек." % [UNIT_LABELS[key], amount]); return
    p.treasury -= price; p[key] += float(amount)
    if key == "missile": _push_news("%s увеличивает запас «%s» на %d. Население не используется." % [p.name, UNIT_LABELS[key], amount])
    else: _push_news("%s увеличивает «%s» на %d человек." % [p.name, UNIT_LABELS[key], amount])
    _refresh_all()
