...
func _bot_tick() -> void:
    ...
    for id in countries.keys():
        ...
        # A country with an unsustainable army enters recovery instead of spending.
        if _bot_needs_recovery(c):
            c.economic_recovery = true
            _bot_demobilize_step(c)
        ...
        var reserve_ready_for_economy: bool = reserve >= effective
        var reserve_ready_for_military: bool = reserve >= effective * 3.0
        var sustainable_surplus: bool = net_income >= effective * 0.20
        var roll := randf()

        if roll < 0.42 and spendable > 150.0 and reserve_ready_for_military and sustainable_surplus:
        ...
        elif roll < 0.62 and spendable >= 250.0 and reserve_ready_for_economy:
            c.treasury -= 250.0
            c.economy += 1.0
            c.income *= 1.01
            _mark_activity(id, ACTIVITY_ECONOMY)
            ...
            _bot_diplomacy(id)
        elif roll > 0.97 and sustainable_surplus:
            _bot_may_attack(id)
    _refresh_all()

func _war_personality_score(ai_type: String) -> float:
    match ai_type:
        "агрессивный": return 20.0
        "авантюрный": return 10.0
        "осторожный": return -20.0
        "дипломатический": return -15.0
        "экономический": return -10.0
        _: return 0.0

func _allied_power(country_id: String) -> float:
    var total: float = 0.0
    for ally_id in countries[country_id].get("allies", []):
        if countries.has(ally_id): total += _total_power(countries[ally_id])
    return total
...
func _war_target_score(attacker_id: String, defender_id: String) -> float:
    ...
    if relations <= -60: score += 25.0
    elif relations <= -30: score += 15.0
    elif relations >= 40: score -= 35.0
    elif relations >= 0: score -= 12.0
    var strength_ratio: float = _total_power(a) / maxf(1.0, _total_power(d))
    if strength_ratio >= 1.50: score += 25.0
    elif strength_ratio >= 1.20: score += 15.0
    elif strength_ratio < 0.70: score -= 40.0
    elif strength_ratio < 0.90: score -= 25.0
    ...
    if reserve_cycles >= 8.0: score += 10.0
    elif reserve_cycles < 4.0: score -= 15.0
    if float(a.economy) < 80.0: score -= 15.0
    elif float(a.economy) > 120.0: score += 5.0
    var defender_allies_power: float = _allied_power(defender_id)
    if defender_allies_power > _total_power(d) * 0.50: score -= 10.0
    if defender_allies_power > _total_power(a): score -= 25.0
    if float(d.treasury) > float(a.treasury) * 1.5: score += 5.0
    return score + randf_range(0.0, 8.0)

func _bot_may_attack(attacker_id: String) -> void:
    if int(countries[attacker_id].get("war_cooldown", 0)) > 0: return
    var a: Dictionary = countries[attacker_id]
    if _bot_needs_recovery(a): return
    var minimum_reserve: float = maxf(100.0, _effective_income(a) * WAR_MIN_RESERVE_CYCLES)
    if float(a.treasury) < minimum_reserve: return
    if float(a.war_fatigue) >= 70.0: return
    var candidates: Array = countries.keys().filter(func(x): return x != attacker_id and not _are_allies(attacker_id, x))
    ...
    if best_target == "" or best_score < WAR_SCORE_THRESHOLD: return
...
func _activity_icon(id: String) -> String:
    match str(countries[id].get("activity", ACTIVITY_IDLE)):
        ACTIVITY_ECONOMY: return "🟢"
        ACTIVITY_MILITARY: return "🔴"
        ACTIVITY_WAR: return "⚔️"
        ACTIVITY_ALLIANCE: return "🔵"
        _: return "⚪"
...
func _refresh_news() -> void:
    ...
    for item in news:
        ...
        if text.begins_with("БИТВА:"):
            ...
            battle_count += 1
    if battle_count == 0: _add_label(news_box, "Боевых столкновений пока не было.", 16)

func _resolve_battle(attacker_id: String, defender_id: String, attack_fraction: float, silent_bot: bool = false) -> void:
    if _are_allies(attacker_id, defender_id): return
