...
- Lobby commands: `create_room`, `join_room`, `select_country`,
  `set_difficulty`, `start_room`. Only the host can change bot difficulty.
...
- Authoritative `game_state` snapshots contain revision, server timer, players,
  AI countries, selected difficulty, territories (including neutral-capture
  protection), moving units, phase, and winner.
...
The server runs Easy, Medium, or Hard AI using the host's lobby selection. New
enemy orders cannot target a territory for 60 seconds after it is captured from
a neutral owner; that timer is part of the authoritative snapshot.
