...
func _draw_sea() -> void:
    draw_rect(Rect2(Vector2.ZERO,size),Color(0.004,0.014,0.027))
    var center:=size*Vector2(0.57,0.47)
    var max_r:float=maxf(size.x,size.y)*0.86
    for i in range(12,0,-1):
        var f:float=float(i)/12.0
        draw_circle(center,max_r*f,Color(0.025,0.105,0.155,0.012+(1.0-f)*0.012))
    var edge:float=34.0
    for i in range(8):
        var inset:float=float(i)*edge
        draw_rect(Rect2(Vector2(inset,inset),size-Vector2(inset*2.0,inset*2.0)),Color(0.0,0.0,0.0,0.026-float(i)*0.0025),false,edge)
    var y:float=32.0
    while y<size.y:
        var x:float=32.0
        while x<size.x:
            draw_circle(Vector2(x,y),0.7,Color(0.28,0.50,0.62,0.035));x+=64.0
        y+=64.0

func _draw_territory(poly:PackedVector2Array,owner:String,iso:String) -> void:
    var center:Vector2=_bounds(poly).get_center()
    if owner=="NEUTRAL":
        draw_colored_polygon(poly,Color(0.025,0.030,0.036))
        draw_colored_polygon(_scaled_poly(poly,center,0.965),Color(0.041,0.048,0.055));return
    var c:Color=_owner_color(owner)
    var selected:bool=iso==drag_source
    draw_colored_polygon(poly,c.darkened(0.52 if not selected else 0.43))
    draw_colored_polygon(_scaled_poly(poly,center,0.955),c.darkened(0.33 if not selected else 0.24))
    var core:Color=c.darkened(0.18 if not selected else 0.10)
    draw_colored_polygon(_scaled_poly(poly,center,0.78),Color(core.r,core.g,core.b,0.70))
    draw_colored_polygon(_scaled_poly(poly,center,0.58),Color(c.r,c.g,c.b,0.13))

func _draw_border(poly:PackedVector2Array,owner:String,iso:String) -> void:
    var is_player:bool=owner==player_country
    var selected:bool=iso==drag_source
    var glow:Color;var core:Color;var gw:float;var cw:float
    if is_player:
        glow=Color(1.0,0.67,0.10,0.36 if not selected else 0.58);core=Color(1.0,0.84,0.30,0.96);gw=7.0 if selected else 5.0;cw=2.5 if selected else 1.8
    elif owner=="NEUTRAL":
        glow=Color(0.18,0.30,0.38,0.08);core=Color(0.27,0.36,0.42,0.48);gw=2.2;cw=0.8
    else:
    ...
    for i in range(poly.size()):
        ...
        draw_line(a,b,glow,gw,true);draw_line(a,b,core,cw,true)
...
func _badge_box(bg:Color,border:Color,radius:int) -> StyleBoxFlat:
    var box:=StyleBoxFlat.new();box.bg_color=bg;box.border_color=border;box.set_border_width_all(1)
...
func _draw_badge(center:Vector2,w:float,h:float,border:=Color(0.34,0.55,0.66,0.42)) -> void:
    draw_style_box(_badge_box(Color(0.003,0.012,0.022,0.86),border,9),Rect2(center-Vector2(w*0.5,h*0.5),Vector2(w,h)))
...
func _draw_marker(iso:String,center:Vector2) -> void:
    ...
    if owner=="NEUTRAL":
        draw_string_outline(ThemeDB.fallback_font,c+Vector2(-22,6),count,HORIZONTAL_ALIGNMENT_CENTER,44,15,3,Color(0,0,0,0.94))
        draw_string(ThemeDB.fallback_font,c+Vector2(-22,6),count,HORIZONTAL_ALIGNMENT_CENTER,44,15,Color(0.90,0.93,0.95));return
    var flag:String=str(FLAGS.get(owner,""))
    var show_name:bool=ACTIVE_IDS.has(iso) and zoom>=1.04
    var w:float=116.0 if show_name else 88.0
    var flag_y:float=-13.0 if show_name else -8.0
    draw_string_outline(ThemeDB.fallback_font,c+Vector2(-w*0.5,flag_y),flag,HORIZONTAL_ALIGNMENT_CENTER,w,25,4,Color(0,0,0,0.92))
    draw_string(ThemeDB.fallback_font,c+Vector2(-w*0.5,flag_y),flag,HORIZONTAL_ALIGNMENT_CENTER,w,25,Color.WHITE)
    if show_name:
        var title:String=str(NAMES.get(iso,""))
        draw_string_outline(ThemeDB.fallback_font,c+Vector2(-w*0.5,10),title,HORIZONTAL_ALIGNMENT_CENTER,w,13,3,Color(0,0,0,0.96))
        draw_string(ThemeDB.fallback_font,c+Vector2(-w*0.5,10),title,HORIZONTAL_ALIGNMENT_CENTER,w,13,Color(0.94,0.97,1.0))
    var cy:float=30.0 if show_name else 19.0
    draw_string_outline(ThemeDB.fallback_font,c+Vector2(-w*0.5,cy),count,HORIZONTAL_ALIGNMENT_CENTER,w,18,4,Color(0,0,0,0.96))
...
func _curve_points(a:Vector2,b:Vector2) -> PackedVector2Array:
    ...
    for i in range(21):
        var t:float=float(i)/20.0;points.append((1.0-t)*(1.0-t)*a+2.0*(1.0-t)*t*control+t*t*b)
    return points
...
func _draw_army(center:Vector2,amount:int,owner:String,direction:Vector2) -> void:
    ...
    for i in range(n):
        var seed:float=float(i);var along:float=(fmod(seed*17.37,100.0)/100.0-0.5)*78.0;var across:float=(fmod(seed*31.91,100.0)/100.0-0.5)*25.0;var p:Vector2=center-direction*along+side*across
...
func _draw() -> void:
    ...
    for feature in map_features:
        ...
        for ring in _rings(feature):
            ...
            var bb:Rect2=_bounds(p);var area:float=bb.size.x*bb.size.y
    ...
    if drag_source!="" and feature_centers.has(drag_source) and not camera_gesture:
        ...
        var source:Vector2=Vector2(feature_centers[drag_source]);var route:PackedVector2Array=_curve_points(source,finger);var route_color:Color=_owner_color(player_country)
        draw_polyline(route,Color(route_color.r,route_color.g,route_color.b,0.13),5.0,true);draw_polyline(route,Color(route_color.r*0.55+0.45,route_color.g*0.55+0.45,route_color.b*0.55+0.45,0.82),1.5,true)
...
func _ai_attack() -> void:
    ...
    for iso in territories.keys():
        ...
        var dist:float=Vector2(feature_centers[source]).distance_to(Vector2(feature_centers[iso]));var neutral_bonus:float=0.65 if str(target.owner)=="NEUTRAL" else 1.0;var score:float=(float(target.army)+20.0)*neutral_bonus+dist*0.09
...
func _begin_camera_gesture() -> void:
    ...
    camera_gesture=true;suppress_single_touch=true;drag_source=""
    var a:Vector2=pair[0];var b:Vector2=pair[1]
    pinch_distance=a.distance_to(b);pinch_midpoint=(a+b)*0.5

func _clamp_pan(value:Vector2,current_zoom:float) -> Vector2:
    if current_zoom<=1.001:return Vector2.ZERO
    var half_extra:Vector2=size*(current_zoom-1.0)*0.5
    var limit_x:float=maxf(0.0,half_extra.x-48.0);var limit_y:float=maxf(0.0,half_extra.y-48.0)
...
func _update_camera_gesture() -> void:
    ...
    var old_zoom:float=zoom;var new_zoom:float=clampf(old_zoom*distance/pinch_distance,1.0,2.8)
    var new_pan:Vector2=pan+(midpoint-pinch_midpoint)
    if old_zoom>0.0 and not is_equal_approx(new_zoom,old_zoom):
        var focus:Vector2=pinch_midpoint-size*0.5-pan
        new_pan-=focus*(new_zoom/old_zoom-1.0)
    zoom=new_zoom
...
func _gui_input(event:InputEvent) -> void:
    ...
    if event is InputEventScreenTouch:
        if event.pressed:
            ...
            if touches.size()==1 and not suppress_single_touch:
                drag_source=_hit(event.position)
            elif touches.size()==2:
                _begin_camera_gesture()
            elif touches.size()>2:
                camera_gesture=false;suppress_single_touch=true;drag_source=""
        else:
            var was_camera:bool=camera_gesture or suppress_single_touch
            ...
            if touches.size()<2:
                camera_gesture=false;pinch_distance=0.0;pinch_midpoint=Vector2.ZERO;drag_source=""
            if touches.is_empty():suppress_single_touch=false
