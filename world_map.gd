...
func _feature_iso(props: Dictionary) -> String:
    ...
    if iso == "" or iso == "-99":
        ...
        var a3_to_a2 := {"FRA":"FR","RUS":"RU","UKR":"UA","POL":"PL","DEU":"DE","GBR":"GB","CHN":"CN","IND":"IN","IRN":"IR","JPN":"JP"}
...
func _load_geojson() -> void:
    ...
    for feature in parsed.get("features", []):
        ...
        if touches and (continent == "Europe" or continent == "Asia" or _feature_iso(props) in ["RU"]): map_features.append(feature)
...
func _poly_bounds(poly: PackedVector2Array) -> Rect2:
    ...
    for p in poly:
        min_x=minf(min_x,p.x); max_x=maxf(max_x,p.x); min_y=minf(min_y,p.y); max_y=maxf(max_y,p.y)
...
func _draw_horizontal_flag(poly: PackedVector2Array, colors: Array) -> void:
    ...
    for band in range(3):
        var y1 := b.position.y+b.size.y*float(band)/3.0; var y2 := b.position.y+b.size.y*float(band+1)/3.0
        var rect := PackedVector2Array([Vector2(b.position.x-4,y1),Vector2(b.end.x+4,y1),Vector2(b.end.x+4,y2),Vector2(b.position.x-4,y2)])
        _clip_fill(poly,rect,colors[band])

func _draw_vertical_flag(poly: PackedVector2Array, colors: Array) -> void:
    var b := _poly_bounds(poly)
    for band in range(3):
        var x1 := b.position.x+b.size.x*float(band)/3.0; var x2 := b.position.x+b.size.x*float(band+1)/3.0
        var rect := PackedVector2Array([Vector2(x1,b.position.y-4),Vector2(x2,b.position.y-4),Vector2(x2,b.end.y+4),Vector2(x1,b.end.y+4)])
        _clip_fill(poly,rect,colors[band])

func _circle_polygon(center: Vector2, radius: float, steps: int = 24) -> PackedVector2Array:
    var points := PackedVector2Array()
    for i in range(steps):
        var angle := TAU*float(i)/float(steps); points.append(center+Vector2(cos(angle),sin(angle))*radius)
    return points

func _draw_flag_polygon(poly: PackedVector2Array, iso: String) -> void:
    if not countries.has(iso) or not FLAG_COLORS.has(iso): draw_colored_polygon(poly,Color(0.26,0.29,0.27)); return
    var colors: Array = FLAG_COLORS[iso]
    if iso == "FR": _draw_vertical_flag(poly,colors); return
    if iso == "JP":
        draw_colored_polygon(poly,Color.WHITE); var b := _poly_bounds(poly); _clip_fill(poly,_circle_polygon(b.get_center(),minf(b.size.x,b.size.y)*0.22),colors[1]); return
    if iso == "CN":
        draw_colored_polygon(poly,colors[0]); var cb := _poly_bounds(poly); var ec := cb.position+Vector2(cb.size.x*0.23,cb.size.y*0.28); _clip_fill(poly,_circle_polygon(ec,minf(cb.size.x,cb.size.y)*0.08,10),colors[1]); return
    if iso == "GB":
        draw_colored_polygon(poly,colors[0]); var ub := _poly_bounds(poly); var cx:=ub.get_center().x; var cy:=ub.get_center().y
        _clip_fill(poly,PackedVector2Array([Vector2(cx-ub.size.x*.10,ub.position.y),Vector2(cx+ub.size.x*.10,ub.position.y),Vector2(cx+ub.size.x*.10,ub.end.y),Vector2(cx-ub.size.x*.10,ub.end.y)]),Color.WHITE)
        _clip_fill(poly,PackedVector2Array([Vector2(ub.position.x,cy-ub.size.y*.10),Vector2(ub.end.x,cy-ub.size.y*.10),Vector2(ub.end.x,cy+ub.size.y*.10),Vector2(ub.position.x,cy+ub.size.y*.10)]),Color.WHITE)
        _clip_fill(poly,PackedVector2Array([Vector2(cx-ub.size.x*.05,ub.position.y),Vector2(cx+ub.size.x*.05,ub.position.y),Vector2(cx+ub.size.x*.05,ub.end.y),Vector2(cx-ub.size.x*.05,ub.end.y)]),colors[2])
        _clip_fill(poly,PackedVector2Array([Vector2(ub.position.x,cy-ub.size.y*.05),Vector2(ub.end.x,cy-ub.size.y*.05),Vector2(ub.end.x,cy+ub.size.y*.05),Vector2(ub.position.x,cy+ub.size.y*.05)]),colors[2]); return
    _draw_horizontal_flag(poly,colors)

func _draw() -> void:
    draw_rect(Rect2(Vector2.ZERO,size),Color(0.035,0.12,0.20,1.0)); hit_polygons.clear()
    for feature in map_features:
        var props: Dictionary = feature.get("properties", {}); var iso := _feature_iso(props); var feature_polys: Array = []
        for ring in _geometry_rings(feature):
            var poly := _screen_poly(ring)
            if poly.size() < 3: continue
            _draw_flag_polygon(poly,iso)
            var border:=Color(0.72,0.76,0.70,0.85); var width:=1.0
...
func _draw_active_labels() -> void:
    var labels := {"GB":Vector2(-3,55),"FR":Vector2(2,46),"DE":Vector2(10.5,51),"PL":Vector2(19,52),"UA":Vector2(31,49),"RU":Vector2(67,60),"IR":Vector2(53,32),"IN":Vector2(79,22.5),"CN":Vector2(104,35),"JP":Vector2(138,37)}
    for iso in labels.keys():
        ...
        var text_pos := pos-Vector2(42,0)
        draw_string_outline(ThemeDB.fallback_font,text_pos,title,HORIZONTAL_ALIGNMENT_CENTER,84,13,3,Color(0.96,0.96,0.92))
        draw_string(ThemeDB.fallback_font,text_pos,title,HORIZONTAL_ALIGNMENT_CENTER,84,13,Color(0.02,0.02,0.02))
        ...
        for iso in hit_polygons.keys():
            for poly in hit_polygons[iso]:
                if Geometry2D.is_point_in_polygon(event.position,poly):
                    country_clicked.emit(iso); accept_event(); return
