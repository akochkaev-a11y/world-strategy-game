...
func _draw()->void:
    ...
    if drag_source!="" and feature_centers.has(drag_source) and not camera_gesture:
        ...
        var source:Vector2=Vector2(feature_centers[drag_source]);var route:PackedVector2Array=_curve_points(source,finger);var route_color:Color=_owner_color(player_country);draw_polyline(route,Color(route_color.r,route_color.g,route_color.b,0.13),5.0,true);draw_polyline(route,Color(route_color.r*0.55+0.45,route_color.g*0.55+0.45,route_color.b*0.55+0.45,0.82),1.5,true);var hover:String=_hit(finger)
