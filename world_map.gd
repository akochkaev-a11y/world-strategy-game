...
func setup(_data: Dictionary, selected: String) -> void:
 player_country=selected; _load_geojson(); _ensure_territories(); queue_redraw()
...
func _load_geojson() -> void:
 ...
 for feature in parsed.get("features",[]):
  var props:Dictionary=feature.get("properties",{}); var continent:=str(props.get("CONTINENT",""))
...
func _ensure_territories()->void:
 for feature in map_features:
  ...
  var active:=ACTIVE_IDS.has(iso); territories[iso]={"owner":iso if active else "NEUTRAL","army":START_ARMY,"active":active}; growth_fraction[iso]=0.0
func grow_armies()->void:
 for iso in territories.keys():
  var t:Dictionary=territories[iso]; var rate:float=ACTIVE_GROWTH if str(t.owner)!="NEUTRAL" else NEUTRAL_GROWTH; growth_fraction[iso]=float(growth_fraction.get(iso,0.0))+rate; var whole:=int(floor(float(growth_fraction[iso])))
...
func _owner_color(owner:String)->Color:
 var colors:={"RU":Color(0.18,0.38,0.78),"UA":Color(0.10,0.52,0.86),"PL":Color(0.85,0.25,0.35),"FR":Color(0.18,0.30,0.72),"DE":Color(0.18,0.18,0.18),"GB":Color(0.25,0.20,0.62),"CN":Color(0.82,0.10,0.10),"IN":Color(0.92,0.50,0.12),"IR":Color(0.10,0.55,0.25),"JP":Color(0.92,0.92,0.92)};return colors.get(owner,Color(0.34,0.36,0.38))
func _draw_soldiers(center:Vector2,amount:int,owner:String,direction:Vector2)->void:
 if amount<=0:return
 var cols:=int(ceil(sqrt(float(amount))));var rows:=int(ceil(float(amount)/float(cols)));var side:=Vector2(-direction.y,direction.x);var back:=-direction;var start_side:=-float(cols-1)*SOLDIER_SPACING*0.5;var start_back:=-float(rows-1)*SOLDIER_SPACING*0.5
 for i in range(amount):
  var col:=i%cols;var row:=i/cols;var p:=center+side*(start_side+float(col)*SOLDIER_SPACING)+back*(start_back+float(row)*SOLDIER_SPACING);draw_circle(p,SOLDIER_RADIUS,_owner_color(owner));draw_circle(p,SOLDIER_RADIUS,Color.WHITE,false,1.0)
func _draw()->void:
 draw_rect(Rect2(Vector2.ZERO,size),Color(0.035,0.10,0.16));hit_polygons.clear();feature_centers.clear();var best_area:Dictionary={}
 for feature in map_features:
  var iso:=_feature_iso(feature.get("properties",{}))
  ...
  for ring in _rings(feature):
   ...
   var owner:=str(territories[iso].owner);draw_colored_polygon(p,_owner_color(owner))
   ...
   if area>float(best_area.get(iso,0.0)):best_area[iso]=area;feature_centers[iso]=bb.get_center()
   polys.append(p)
  if not polys.is_empty():hit_polygons[iso]=polys
 for iso in feature_centers.keys():
  var t:Dictionary=territories[iso];var c:Vector2=feature_centers[iso];var owner:=str(t.owner);var title:String=str(NAMES.get(iso,"")) if ACTIVE_IDS.has(iso) else "";var flag:String=str(FLAGS.get(owner,"")) if owner!="NEUTRAL" else "";var line1:=(flag+" "+title).strip_edges()
 ...
 for a in armies:
  var pos:Vector2=a.pos;var target:=str(a.target);var direction:=Vector2.RIGHT
...
func _move_armies(delta:float)->void:
 for i in range(armies.size()-1,-1,-1):
  ...
  var dest:Vector2=feature_centers[target];var pos:Vector2=a.pos;var d:=pos.distance_to(dest)
  if d<=ARMY_SPEED*delta:_arrive(i);continue
...
func _arrive(index:int)->void:
 ...
 if attackers>defenders:t.owner=str(a.owner);t.army=attackers-defenders;t.active=true
...
func _ai_attack()->void:
 ...
 var source:=str(sources[randi()%sources.size()]);var owner:=str(territories[source].owner);var targets:Array=[]
 for iso in territories.keys():
...
func _gui_input(event:InputEvent)->void:
 if event is InputEventScreenTouch:
  ...
  if touches.size()==2:
   ...
   if pinch_distance>0.0:var old_zoom:=zoom;zoom=clampf(zoom*(new_dist/pinch_distance),0.8,5.0);pan=(pan+(pinch_center-size*0.5)*(1.0-old_zoom/zoom))+(new_center-pinch_center)
